...
```mermaid
flowchart LR
    A[Oracle BMIS] --> B[Bronze BMIS / Spark ingest]
    C[CSV file / bims_umumi_gostericiler.csv] --> D[Bronze BMIS bims_difference]
    B --> E[Silver BMIS / dbt]
    D --> F[Silver snapshot bims_difference]
    E --> G[Gold BMIS / dbt]
    G --> H[Gold Tableau / dbt]
    F --> H
    H --> I[Tableau / reporting]
```
